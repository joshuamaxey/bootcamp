/* For instructions on how to populate your database with the data in an SQL file, see the pseudocode below the SQL code here! */
...
/* Now that we have our file set up, we can insert it into a database using the following commands:

First, open SQLite3 using the command 'sqlite3' (make sure you're in the same directory as this create-table.sql file)

Then use the following command to populate your database with the data in this file: '.read create-table.sql'

Now verify that you've inserted your file into the database by runnign this command: 'SELECT * FROM pies;' You should see all of the pies display as text in the terminal

To view the shcema that you've created for the pies data, run this command: '.schema'

if you want to reset your database using this file, run 'DOP TABLE pies' and then re-create your database through the CLI. All in all, enter this into your CLI:
...
Then just run '.tables' to make sure that it worked. If you see 'pies' in the terminal, you were successful.
*/
