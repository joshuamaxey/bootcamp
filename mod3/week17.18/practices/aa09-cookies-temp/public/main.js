...
function restoreTheme() {
    const kvp = document.cookie.split("; ");

    const themeCookie = kvp.find((el) => el.startsWith("theme="));

    if (themeCookie) {
        const storedTheme = themeCookie.split("=")[1];
...
function restoreName() {
    ...
    const displayCookie = kvp.find((el) => el.startsWith("displayName="));

    if (displayCookie) {
        const displayName = displayCookie.split("=")[1];
