...
def sort_teachers_by_classroom_capacity(card_list):
    sorted_teachers = sorted(card_list, key=lambda card: card["classroom"]["capacity"])
    print(sorted_teachers) # correctly returns the teachers sorted by classroom capacity in ascending order
    names = [card["name"] for card in sorted_teachers]
    return names
